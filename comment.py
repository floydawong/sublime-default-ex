...
TEMPLATE_COMMENT = " {{comment by Floyda}} "
    ...
    for suffix in suffixes:
        ...
        if start and end:
            block_comments.append(
                (start.strip() + TEMPLATE_COMMENT, end, disable_indent == "yes")
            )
            block_comments.append((start.strip(), end.strip(), disable_indent == "yes"))
        elif start:
            line_comments.append(
                (start.strip() + TEMPLATE_COMMENT, disable_indent == "yes")
            )
...
class ToggleCommentCommand(sublime_plugin.TextCommand):
    ...
    def line_comment_region(self, view, edit, line_comment_data, region):
        ...
        non_empty_start_positions = list(
            filter(lambda p: has_non_white_space_on_line(view, p), start_positions)
        )
